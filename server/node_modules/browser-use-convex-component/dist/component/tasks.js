...
export const saveTaskSteps = internalMutation({
    ...
    handler: async (ctx, args) => {
        for (const step of args.steps) {
            const existing = await ctx.db
                .query("taskSteps")
                .withIndex("byExternalTaskId", (q) => q.eq("externalTaskId", args.externalTaskId))
...
export const list = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.sessionId) {
            q = ctx.db
                .query("tasks")
                .withIndex("bySessionId", (idx) => idx.eq("sessionId", args.sessionId));
            if (args.status) {
...
export const create = action({
    ...
    handler: async (ctx, args) => {
        ...
        if (taskArgs.sessionId)
            body.sessionId = taskArgs.sessionId;
        if (taskArgs.startUrl)
            body.startUrl = taskArgs.startUrl;
        if (taskArgs.maxSteps)
            body.maxSteps = taskArgs.maxSteps;
        if (taskArgs.flashMode !== undefined)
            body.flashMode = taskArgs.flashMode;
        if (taskArgs.thinking !== undefined)
            body.thinking = taskArgs.thinking;
        if (taskArgs.vision !== undefined)
            body.vision = taskArgs.vision;
        ...
        if (taskArgs.secrets)
            body.secrets = taskArgs.secrets;
        if (taskArgs.allowedDomains)
            body.allowedDomains = taskArgs.allowedDomains;
        if (taskArgs.structuredOutput)
            body.structuredOutput = taskArgs.structuredOutput;
        if (taskArgs.metadata)
            body.metadata = taskArgs.metadata;
        if (taskArgs.judge !== undefined)
            body.judge = taskArgs.judge;
        if (taskArgs.judgeGroundTruth)
            body.judgeGroundTruth = taskArgs.judgeGroundTruth;
        if (taskArgs.judgeLlm)
            body.judgeLlm = taskArgs.judgeLlm;
        if (taskArgs.skillIds)
            body.skillIds = taskArgs.skillIds;
        const response = await browserUseRequest({
            method: "POST",
            path: "/api/v2/tasks",
            apiKey,
            body,
